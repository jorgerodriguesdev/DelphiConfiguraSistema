unit AConfiguracaoImpressaoBarraOS;
...
type
  TFConfiguraImpressaoBarraOS = class(TFormularioPermissao)
...
var
  FConfiguraImpressaoBarraOS: TFConfiguraImpressaoBarraOS;

implementation

uses APrincipal, Constantes, FunSql, ConstMsg, FunString, UnClassesImprimir,
  AImprimeEtiquetaBarraOS, funsistema;

{$R *.DFM}

{ ****************** Na criação do Formulário ******************************** }
procedure TFConfiguraImpressaoBarraOS.FormCreate(Sender: TObject);
begin
...
procedure TFConfiguraImpressaoBarraOS.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFConfiguraImpressaoBarraOS.FormShow(Sender: TObject);
begin
  Self.HelpFile := Varia.PathHelp + 'MaConfSistema.HLP>janela';  // Indica o Paph e o nome do arquivo de Help
end;

procedure TFConfiguraImpressaoBarraOS.BVisualizaClick(Sender: TObject);
begin
  if not VerificaFormCriado('TFImprimeEtiquetaBarraOS') then
     FImprimeEtiquetaBarraOS := TFImprimeEtiquetaBarraOS.CriarSDI(application, '', true);
  FImprimeEtiquetaBarraOS.VisualizaBarra( CAD_DOCI_NRO_DOC.AsInteger);
end;


{**************** fecha formulario ****************************************** }
procedure TFConfiguraImpressaoBarraOS.BFecharClick(Sender: TObject);
begin
  Close;
end;
...
procedure TFConfiguraImpressaoBarraOS.CAD_DOCAfterInsert(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.CAD_DOCBeforePost(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.CAD_DOCAfterPost(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.CAD_DOCAfterScroll(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.CAD_DOCBeforeEdit(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.SetText_SN(Sender: TField;
  const Text: String);
begin
...
{***************** antes do Insert ***************************************** }
procedure TFConfiguraImpressaoBarraOS.MOV_DOCBeforeInsert(DataSet: TDataSet);
begin
  Abort;
end;

{********************* antes do Delete ************************************* }
procedure TFConfiguraImpressaoBarraOS.MOV_DOCBeforeDelete(DataSet: TDataSet);
begin
  Abort;
end;

{************** Garante que o texto digitado ser D ou d ou E ou e ********** }
procedure TFConfiguraImpressaoBarraOS.SetText_DE(Sender: TField;
  const Text: String);
begin
...
procedure TFConfiguraImpressaoBarraOS.MOV_DOCBeforePost(DataSet: TDataSet);
begin
...
procedure TFConfiguraImpressaoBarraOS.DBEditColor1Exit(Sender: TObject);
begin
...
procedure TFConfiguraImpressaoBarraOS.Negativo_keyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = '-' then
   Key := '#';
...
procedure TFConfiguraImpressaoBarraOS.MudaEstado( acao : Boolean );
begin
...
procedure TFConfiguraImpressaoBarraOS.InicializaItens(NroDoc: Integer; Atualizar : Boolean);
...
{**************** cadastrar documento ************************************** }
procedure TFConfiguraImpressaoBarraOS.BCadastrarClick(Sender: TObject);
var
  VpfNovo,
  VpfMsg : string;
begin
...
{******************* altera um documento ************************************* }
procedure TFConfiguraImpressaoBarraOS.BAlterarClick(Sender: TObject);
begin
  if not FPrincipal.BaseDados.InTransaction  then
    FPrincipal.BaseDados.StartTransaction;
  DATA_CAD_DOC.AutoEdit := true;
...
{***************** Excluir documento **************************************** }
procedure TFConfiguraImpressaoBarraOS.BExcluirClick(Sender: TObject);
begin
...
{********************* grava documento ************************************** }
procedure TFConfiguraImpressaoBarraOS.BGravarClick(Sender: TObject);
begin
    try
      if MOV_DOC.State in [dsInsert, dsEdit] then
        MOV_DOC.Post;
...
{******************* Cancela documento ************************************* }
procedure TFConfiguraImpressaoBarraOS.BCancelarClick(Sender: TObject);
begin
   if MOV_DOC.State in [dsInsert, dsEdit] then
    MOV_DOC.Cancel;
  if CAD_DOC.State in [dsInsert, dsEdit] then
...
procedure TFConfiguraImpressaoBarraOS.BExemploClick(Sender: TObject);
begin
  if MOV_DOC.State in [dsInsert, dsEdit] then
    MOV_DOC.Post;
  if CAD_DOC.State in [dsInsert, dsEdit] then
  ...
  if not VerificaFormCriado('TFImprimeEtiquetaBarraOS') then
     FImprimeEtiquetaBarraOS := TFImprimeEtiquetaBarraOS.CriarSDI(application, '', true);
  FImprimeEtiquetaBarraOS.ImprimeBarra( CAD_DOCI_NRO_DOC.AsInteger,1,100);
  FImprimeEtiquetaBarraOS.close;
end;


{***************** cria um novo documento apartir do modelo ****************** }
procedure TFConfiguraImpressaoBarraOS.BModeloClick(Sender: TObject);
var
  Proximo,
  I: Integer;
...
procedure TFConfiguraImpressaoBarraOS.DBEdit9Exit(Sender: TObject);
begin
...
procedure TFConfiguraImpressaoBarraOS.BBAjudaClick(Sender: TObject);
begin
  Application.HelpCommand(HELP_CONTEXT,FConfiguraImpressaoBarraOS.HelpContext);
end;

procedure TFConfiguraImpressaoBarraOS.BitBtn6Click(Sender: TObject);
begin
...
Initialization
  RegisterClasses([TFConfiguraImpressaoBarraOS]);
end.
